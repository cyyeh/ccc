# Phase 7 — Port CPython 3.12 (Design)

**Project:** From-Scratch Computer (directory `ccc/`).
**Phase:** 7 — see `2026-04-23-from-scratch-computer-roadmap.md` (this spec
extends that roadmap; the original document lists five phases plus the
Phase 6 graphics addendum, and this is a second post-roadmap addition —
...
Independent of Phase 4 / 5 / 6 — can land before, after, or in parallel.
...
After Phase 7, ccc's shell (`/bin/sh`) can launch Python programs the
...
browser). Phase 6 added an optional graphics addendum reversing the "no
graphics" non-goal. Phase 7 is the project's first **port** rather than
...
2. **It opens userland to non-Zig contributors.** Today, adding a new
   ccc demo means a Zig recompile cycle. After Phase 7, a 30-line
...
**Cost honesty.** Phase 7 is the first phase whose code volume comes
...
- All Phase 1 e2e tests (`e2e`, `e2e-mul`, `e2e-trap`, `e2e-hello-elf`),
  ...
  pass. **The kernel changes in 7.F (signal mode opt-in) are additive —
  Phase 3.F's `e2e-cancel` continues to assert the default exit-on-^C
  behavior unchanged.**
- New e2e tests pass: `e2e-c-hello` (7.A), `e2e-c-alloc` (7.B),
  `e2e-py-eval` (7.C), `e2e-py-repl` (7.D), `e2e-py-stdlib-smoke` (7.D),
  `e2e-py-pi` (7.E), `e2e-py-life` (7.E), `e2e-py-wc` (7.E),
  `e2e-py-json` (7.E), `e2e-py-exception` (7.E), `e2e-py-interrupt` (7.F),
  `e2e-py-demo` (7.G — the full eight-line scripted session above).
- `riscv-tests` (rv32{ui,um,ua,mi,si} all `-p-*`) still pass.
- `--trace` works across the new picolibc syscall surface; the new
  `--trace-py-opcodes` flag (7.G) emits one line per CPython bytecode
...
- **`socket` / `select` / `_socket` / `_ssl`.** All require Phase 4's
  socket layer. Imported as `ImportError`. A Phase 7.5 plan can wire
  them up after Phase 4 lands.
- **`mmap` module / `os.mmap`.** Requires Phase 6's `mmap` syscall.
  Same shape — Phase 7.5 plan after Phase 6.
...
- **`_decimal` (C-accelerated decimal).** We use `_pydecimal` (the
  ...
  dependency. A 7.5 plan can pull in `libmpdec` if perf matters.
- **GNU readline / `libedit`.** No arrow-key history, no Tab completion.
  ...
  prompt. A 7.5 plan adds a small in-process line editor mapped to
...
- **Web (wasm) integration.** A 10–15 MB ELF + the existing RV32
  ...
  multi-second cold start. A 7.8 polish plan revisits once we know
...
- **`os.spawn*` / `subprocess`.** Phase 3 has `fork`/`execve`/`wait4`,
  ...
  `errno=ENOSYS`. A 7.5 plan adds `pipe` standalone if the perf cost
...
- **Full unicode normalization tables.** CPython embeds the full
  ...
  doesn't impact it but binary size does. A 7.G binary-size pass
...
- **`unittest` / `pytest`.** Test runner support is a Phase 7.5
...
- DRM / Wayland / X11 protocol bridging (Phase 6 territory at most).
...
│   └── #220 sigaction_int_mode  ← NEW in 7.F                  │
│                                                              │
...
A first-pass list (chased by the link errors during 7.C bring-up):
...
Phase 7 ships no line editor. The 7.5 polish plan adds one.

### Signal & ^C model

This is the only kernel change in Phase 7. The mechanism:
...
7. **But the kernel doesn't call user-space signal handlers** —
   nothing in Phase 7 supports asynchronous signal delivery in the
...
The `--trace-py-opcodes` flag is added in 7.G to filter the tracer:
...
│   └── userland/                    NEW (Phase 7 introduces this hierarchy)
│       ├── libc-shim/
...
│   │   ├── py_eval.zig              7.C
│   │   ├── py_repl.zig              7.D
│   │   ├── py_stdlib_smoke.zig      7.D
│   │   ├── py_pi.zig                7.E
│   │   ├── py_life.zig              7.E
│   │   ├── py_wc.zig                7.E
│   │   ├── py_json.zig              7.E
│   │   ├── py_exception.zig         7.E
│   │   ├── py_interrupt.zig         7.F
│   │   ├── py_demo.zig              7.G — full 8-line scripted demo
...
- **7.A — picolibc bring-up + image-size bump.**
  Submodule picolibc, write `libc-shim/{syscall,errno,environ,signal}.c`
  (signal.c here is just stubs; the real path lands in 7.F).
...
- **7.B — dlmalloc + soft-float + Zig build skeleton.**
...
- **7.C — `pyconfig.h` + minimum CPython core.**
...
- **7.D — Frozen stdlib + REPL + smoke test.**
...
- **7.E — Demo programs + JSON + Pi + Life + WC + exceptions.**
...
- **7.F — Signal model + ^C interrupt path.**
  ...
  `--trace-py-opcodes` plumbing waits for 7.G. Milestone:
  `e2e-py-interrupt` passes — boot, `python`, REPL, `>>> while True:
  pass`, scripted `^C`, see `KeyboardInterrupt`, scripted `exit()`,
  clean halt.

- **7.G — Polish: trace integration + binary size + final demo + docs.**
  ...
  bigger pieces of `email`). Update README with Phase 7 status.
  Milestone: `e2e-py-demo` (the eight-line scripted session) passes;
  binary is <15 MB; full Phase 1 + 2 + 3 + 7 e2e suite green.

Plan boundaries are designed so 7.A is shippable as "ccc can run
hand-written C programs with libc support" even if the rest of
Phase 7 stalls — it's Phase 7's analog to 6.A's "framebuffer works
...
- The CPython unit-test corpus (`Lib/test/test_*.py`) is **not** in
  ...
  A 7.5 plan explores a curated test_subset (`test_int.py`,
...
|---|---|---|
| `e2e-c-hello` | hand-crafted `printf("hello, libc\n")` C program runs in ccc, prints exactly that. | 7.A |
| `e2e-c-alloc` | dlmalloc `malloc(1<<20)` → `memset(0xab)` → `free` → `malloc(100KB)` × 10 → free reverse → exit 0. Asserts no kernel panic + exit code 0. | 7.B |
| `e2e-py-eval` | `python -c "print(2 + 2)"` prints `4`. Smallest possible CPython runtime smoke test. | 7.C |
| `e2e-py-repl` | scripted REPL: `>>> 2 ** 200<enter>`, expected line, `>>> exit()<enter>`, exit 0. | 7.D |
| `e2e-py-stdlib-smoke` | runs a 1-line script that imports the entire curated stdlib; asserts no `ImportError`. | 7.D |
| `e2e-py-pi` | `python /usr/lib/demo/pi.py 50` → first 50 digits of π exact. | 7.E |
| `e2e-py-life` | `python /usr/lib/demo/life.py 8` → 8 generations, frame hashes match committed goldens (line-by-line CRC). | 7.E |
| `e2e-py-wc` | `python /usr/lib/demo/wc.py /etc/motd` → ` 19  4  1 /etc/motd`. | 7.E |
| `e2e-py-json` | `json_demo.py` round-trips a structure and asserts equality. Stdout is the pretty-printed JSON. | 7.E |
| `e2e-py-exception` | `python /usr/lib/demo/exc.py` prints `ok`. | 7.E |
| `e2e-py-interrupt` | REPL → infinite loop → `^C` → `KeyboardInterrupt` traceback in stdout → `exit()` → exit 0. Timing-sensitive; harness uses `--input` with deliberate ordering. | 7.F |
| `e2e-py-demo` | the full 8-line §Definition of done scripted session. | 7.G |
...
The 8-line demo run interactively at the end of 7.G. Same policy as
...
- **Bring-up time for 7.C.** The first time CPython compiles + links
  ...
  link-error chase early in 7.B (the "compile one CPython file"
...
- **Stack size.** 256 KB might be too tight for deep regex backtracking
  ...
  `--py-stack-mb` build flag in 7.G; default 256 KB, demos that need
  more bump to 1 MB.
- **`_pydecimal` performance.** Pure-Python decimal is ~10× slower
  than `_decimal`. `pi.py 50` finishes in ~150 ms; `pi.py 500` in
  ~15 s. Acceptable; documented; 7.5 plan can pull `libmpdec`.
...
- **Binary size > 15 MB.** First builds will likely be ~25 MB
  ...
  18–20 MB, 7.G drops less-essential frozen modules (`pathlib` →
...
- **CPython submodule churn.** CPython 3.12.x patch versions may
  ...
  patch (e.g. 3.12.4) at plan start and don't bump within Phase 7;
  bumps are 7.5+ work.
...
- **Reentrancy of `_start`.** ccc's `_start.S` from Phase 3.E doesn't
  ...
  calling `main`. ~30 LoC, lands in 7.A.
- **Zig version churn.** Same as every phase. Re-pin `build.zig.zon`
  at Phase 7 start. Phase 7's compiler-rt soft-float dependency may
...
- **README + roadmap update on approval.** Phase 7 isn't in the
  ...
  Phase 7 is its own spec. Note in the spec's status that approval
